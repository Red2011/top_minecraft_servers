...
export function ClickTheClipboard (text: string): void {
        ...
        <div className="flex flex-col w-full">
            <div className="flex space-x-2 max-sm:flex-col max-sm:space-y-3 max-sm:space-x-0 w-full">
                ...
                <div className="flex flex-col justify-center items-center space-y-3 w-1/3 max-sm:w-full max-sm:flex-row max-sm:space-x-2 max-sm:space-y-0">
                    <div tabIndex={0} onClick={() => { ClickTheClipboard(props.serverLink) }} className="py-1 bg-project_tile w-full max-sm:w-[39%] px-1 flex space-x-0.5 justify-center items-center h-full font-gilroy text-[#4f5b6c] text-lg rounded-lg hover:duration-[1000ms] hover:bg-project_tile_hover group">
                        <span className="max-sm:text-xs max-w-[80%] truncate text-center duration-500 animate-smooth_energy_1s group-focus:opacity-0 group-focus:duration-300">
                            {props.serverLink.toUpperCase()}
                        </span>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 group-focus:opacity-0 animate-smooth_energy_1s duration-500 group-focus:duration-300">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M11.35 3.836c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m8.9-4.414c.376.023.75.05 1.124.08 1.131.094 1.976 1.057 1.976 2.192V16.5A2.25 2.25 0 0118 18.75h-2.25m-7.5-10.5H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V18.75m-7.5-10.5h6.375c.621 0 1.125.504 1.125 1.125v9.375m-8.25-3l1.5 1.5 3-3.75" />
                        </svg>
                        <span className="absolute animate-smooth_energy_1s hidden group-focus:block duration-1000 ">
                            copied
                        </span>
                    </div>
                    <button className="bg-[#2EA548] hover:bg-[#118a25] hover:duration-[1000ms] max-sm:w-[61%] flex space-x-1 justify-center items-center h-full w-full font-sourse_sans_pro text-[#fff] text-lg rounded-lg py-1">
